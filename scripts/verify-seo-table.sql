-- Script de verificação e correção da tabela seo_settings
...
SELECT 
    CASE 
        WHEN COUNT(*) = 0 THEN 'Tabela vazia - inserindo dados padrão'
        ELSE 'Tabela já possui dados'
    END as status,
    COUNT(*) as total_registros
FROM seo_settings;

-- 5. Inserir dados padrão se a tabela estiver vazia
INSERT INTO seo_settings (page_name, title, description, keywords, canonical_url, og_title, og_description, og_image, og_site_name, twitter_title, twitter_description, twitter_image, author, structured_data) 
SELECT * FROM (VALUES
    (
        'home',
...
) AS v(page_name, title, description, keywords, canonical_url, og_title, og_description, og_image, og_site_name, twitter_title, twitter_description, twitter_image, author, structured_data)
WHERE NOT EXISTS (SELECT 1 FROM seo_settings WHERE seo_settings.page_name = v.page_name);
